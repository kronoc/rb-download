...
load_carousel(struct carousel *car)
{
  ...
  gettimeofday(&lasttime, NULL);
    ...
    if (car->nmodules != last_modules_n)
    {
      last_modules_n = car->nmodules;
      gettimeofday(&lasttime, NULL);
    } else if (lasttime.tv_sec > 0) {
      gettimeofday(&curtime, NULL);
      if (curtime.tv_sec - lasttime.tv_sec > 180){//seems no data in 3 mins
        done = true;
        printf("Downloaded %d modules\n", last_modules_n);
      }
    }
	}
	while(!done);
